...
public class PCliente {
    ...
    public DefaultTableModel mostrar(String buscar) {
        ...
        sSQL = "select p.codigoPersona, p.numeroDocumento, p.nombre, p.apellidos from tblPersona p"
                + " where numeroDocumento like '%" + buscar + "%' and p.tipoPersona='C' and p.operatividad='S' order by numeroDocumento desc";
    ...
    public  String[] cargarDatosEspecificos(String codigo){  //fijate que al tipo de dato de retorno
        ...
        sSQL="select per.codigoPersona, per.numeroDocumento, per.apellidos, per.nombre, per.telefono, per.tipoPersona, per.direccion,"
                + " per.correo, per.operatividad, cli.historial , cli.sector FROM tblpersona per inner join tblcliente cli"
                + " on per.codigoPersona= cli.codigoPersona where per.codigoPersona=" +codigo+" and per.tipoPersona='C'";      
        ...
        try {
            ...
            while(rs.next()){
                ...
                datos[9] = rs.getString("historial"); 
                datos[10]= rs.getString("sector");
    ...
    public boolean editar(VCliente datos) {
        ...
        sSQL2 = "update tblcliente set historial=?, sector=? where codigoPersona=?";
        try {
            PreparedStatement pst = cn.prepareStatement(sSQL);
            PreparedStatement pst2 = cn.prepareStatement(sSQL2);
            ...
            pst2.setString(1, datos.getHistorial());
            pst2.setString(2,datos.getSector());
            pst2.setInt(3, datos.getCodigoPersona());
    ...
    public boolean eliminar(VCliente datos) {
     ...
     public DefaultTableModel mostrarCliente(String buscar) {
        ...
        /*Se llama una instancia del objeto tabla y se crean los arrays que llevaran los datos*/
        DefaultTableModel modelo;
        String[] titulos = {"CodigoP","Cedula", "Nombre", "Apellido","Telefono"};
        ...
        /*cadena de consulta*/
        sSQL = "select p.codigoPersona, p.numeroDocumento, p.nombre, p.apellidos, p.telefono from tblPersona p"
                + " where numeroDocumento like '%" + buscar + "%' and p.tipoPersona='C' and p.operatividad='S' order by numeroDocumento desc";
